		...
		modals.forEach(function(modal) {
			...
			modal.addEventListener('toggle-target-after-inactive', function(event) {
				...
				setTimeout(function() {
					...
					_win.scrollTo(
						0,
						Math.abs(_win.yonkeo.scrolled + getAdminBarHeight())
					);
...
yonkeo.modalMenu = {
	...
	expandLevel: function() {
		...
		modalMenus.forEach(function(modalMenu) {
			...
			if (activeMenuItem) {
				yonkeoFindParents(activeMenuItem, 'li').forEach(function(
					element
				) {
	...
	performToggle: function(element, instantly) {
		...
		if (target.classList.contains(activeClass)) {
			target.dispatchEvent(
				yonkeo.createEvent('toggle-target-before-active')
			);
		} else {
			target.dispatchEvent(
				yonkeo.createEvent('toggle-target-before-inactive')
			);
		}
		...
		setTimeout(function() {
			...
			if (target.classList.contains(activeClass)) {
				target.dispatchEvent(
					yonkeo.createEvent('toggle-target-after-active')
				);
			} else {
				target.dispatchEvent(
					yonkeo.createEvent('toggle-target-after-inactive')
				);
			}
		}, timeOutTime);
	},
