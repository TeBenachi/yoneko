=== yoneko ===

Contributors: TeBenachi
Tags: Accessibility Ready, custom-logo, custom-menu, featured-images, threaded-comments, Two Columns, Left Sidebar, Sticky Post, Theme Options, Blog, translation-ready
Requires at least: 5.2
Tested up to: 5.6
Requires PHP: 7.4
Stable tag: 1.0.6
License: GNU General Public License v2 or later
License URI: LICENSE

== Description ==

Yoneko is named after my grandmother for her love of writing. Yoneko is suitable for bloggers and educators who want to focus on writing. The unique homepage straightforwardly displays the subject lines without thumbnails for easy scanning.  This distinctive layout also allows readers to find what they want quickly. Yoneko is Japanese typography in mind.  It comes with pre-installed google fonts, including six Japanese fonts, Noto Sans JP, Noto Serif JP, M PLUS Rounded 1c, Kosugi Maru, Sawarabi Mincho, and Sawarabi Gothic, as well as three sans-serif fonts, Lato, Roboto, and Merriweather. 

== Frequently Asked Questions ==

= Is yoneko compatible with IE11? =

Yoneko is developed using Tailwind CSS version 2.0, which no longer support IE11. 

== Changelog ==

= 1.0.6 - Feburary 2, 2021 =
* Fix search form
* Fix accesssibility issues on navigation

= 1.0.5 - January 8, 2021 =
* Fix admin hover/focus link color
* Fix verison prefix 
* Fix search form coding issue and escape function
* Fix search form placeholder text color contrast
* Fix search form accessibility and styling issues
* Fix screenshot image size
* Add filter for posts without titles
* Fix font customizer
* Fix accessibility issues on navigation
* Remove Main landmark. There were two main landmarks 
* Retain underline the text links in the content area
* Update readme.txt 

= 1.0.4 - January 3, 2021 =
* Fix accessibility issues

= 1.0.3 - December 23, 2020 =
* Fix google fonts

= 1.0.2 - December 23, 2020 =
* Fix accessibility ARIA issues
* Update credits on reademe.txt
* Fix table formatting

= 1.0.1 - December 22, 2020 =
* Edit Japanese translation

= 1.0 - December 20, 2020 =
* Initial release


== Credits ==

Underscores _s theme 
Copyright:  2012-2020 Automattic, Inc.
License:    GPLv2 or later https://www.gnu.org/licenses/gpl-2.0.html
Source:     https://underscores.me/ 

normalize.css 
Copyright:  2012-2018 Nicolas Gallagher and Jonathan Neal
License:    MIT Licensed https://opensource.org/licenses/MIT
Source:     https://necolas.github.io/normalize.css/

Twenty Twenty 
Copyright:  Copyright 2019-2020 WordPress.org
License:    GNU General Public License v2 or later http://www.gnu.org/licenses/gpl-2.0.html
Source:     https://wordpress.org/themes/twentytwenty/

Stp Accessibility
Copyright:  Joseph LoPreste
License:    GNU General Public License v2 or later http://www.gnu.org/licenses/gpl-2.0.html
Source:     https://www.stpetedesign.com

Tailwindcss
Copyright:  Adam Wathan and Jonathan Reinink
License:    MIT Licensed https://github.com/tailwindlabs/tailwindcss/blob/master/LICENSE
Source:     https://tailwindcss.com/

Heroicons
Copyright:  Steve Schoger
License:    MIT Licensed https://github.com/tailwindlabs/heroicons/blob/master/LICENSE
Source:     https://heroicons.com/

Google fonts (Noto Sans JP, Noto Serif JP, M PLUS Rounded 1c, Sawarabi Mincho, Sawarabi Gothic )
License:    Open Font License  https://scripts.sil.org/cms/scripts/page.php?site_id=nrsi&id=OFL
Source:     https://fonts.google.com/

Google fonts (Kosugi Maru)
License:    Apache License, Version 2.0 http://www.apache.org/licenses/LICENSE-2.0
Source:     https://fonts.google.com/